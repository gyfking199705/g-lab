...
- **展示**：卡片画廊 + 详情抽屉；左侧按分类/技巧/收藏过滤，顶部全文搜索与排序。
- **复用**：详情里填入 `{{变量}}` 即时预览，一键复制「正文」或「System+User 完整」prompt。
- **管理**：新增 / 编辑 / 删除 / 收藏；网格与列表两种视图。
- **可移植**：导出带 `format: prompt-lab/v1` 的 JSON；导入支持本工具导出对象或裸 prompt 数组（按 id 去重）。
- 首次打开内置 8 条精选示例（覆盖代码评审、摘要、CoT、Few-shot 抽取、ReAct、苏格拉底教学等范式）。
...
└── src/
    ├── schema.js / .test.js   # 数据模型、过滤、导入导出（纯函数）
    ...
    ├── App.jsx                # 顶层状态与布局
    ├── bootstrap.jsx          # 打包入口
    └── components/            # Sidebar / PromptCard / PromptDetail / PromptEditor
```
