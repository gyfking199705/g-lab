...
- **质量体检**：按业界最佳实践对 prompt 打分（角色 / 任务清晰 / 输出格式 / 抗幻觉 / 示例 / 变量 / 可检索 7 项），详情看清单与改进建议，编辑器底部实时显示得分。**点击未达标项**即跳转到编辑器对应字段并高亮，照着补就能提分。
- **管理**：新增 / 编辑 / 删除 / 收藏 / 克隆（副本）；复制为 Markdown；网格与列表两种视图。
- **可移植**：导出菜单含两种格式——JSON 备份（带 `format: prompt-lab/v1`，可再导入）与整库 Markdown 文档（带分组目录，便于分享/归档）；导入支持本工具导出对象或裸 prompt 数组（按 id 去重）。
