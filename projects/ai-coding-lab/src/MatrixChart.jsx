...
import { LEVEL } from '../data/practices.js';
...
const CAT_COLOR = {
  paradigm: '#CC785C', workflow: '#6E9079', technique: '#BE9356',
  tooling: '#7C8AAE', guardrail: '#BC6055',
};
  ...
  return (
    <div className="acl-matrix">
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" role="img" aria-label="影响力与落地成本四象限图">
        ...
        {/* 轴标签 */}
        <text x={PAD} y={PAD - 16} className="acl-mx-quad">⬑ 高影响 · 低成本（优先采用）</text>
        <text x={W - PAD} y={H - PAD + 30} className="acl-mx-axis" textAnchor="end">落地成本 →</text>
        <text x={PAD - 16} y={PAD - 2} className="acl-mx-axis" transform={`rotate(-90 ${PAD - 16} ${PAD - 2})`}>影响力 →</text>
        <text x={PAD} y={H - PAD + 18} className="acl-mx-tick">低</text>
        <text x={W - PAD} y={H - PAD + 18} className="acl-mx-tick" textAnchor="end">高</text>
        <text x={PAD - 12} y={H - PAD} className="acl-mx-tick" textAnchor="end">低</text>
        <text x={PAD - 12} y={PAD + 6} className="acl-mx-tick" textAnchor="end">高</text>
        ...
        {pts.map(({ it, dx, dy }) => {
          ...
          return (
            <g key={it.id} className="acl-mx-pt" onClick={() => onPick && onPick(it)}>
      ...
      <div className="acl-mx-legend">
        {Object.entries(CAT_COLOR).map(([k, c]) => (
          <span key={k} className="acl-mx-leg"><i style={{ background: c }} />{LABEL[k]}</span>
        ))}
        <span className="acl-mx-hint">点击圆点查看详情</span>
      </div>
    </div>
  );
}

const LABEL = { paradigm: '范式', workflow: '工作流', technique: '提效技巧', tooling: '工具', guardrail: '护栏' };
