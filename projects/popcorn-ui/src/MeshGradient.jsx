...
const CSS = `
.popcorn-mesh{position:relative;overflow:hidden;border-radius:16px;background-color:#1b1a17;}
.popcorn-mesh-content{position:relative;z-index:1;width:100%;height:100%;}
`;

export default function MeshGradient({ colors, speed = 0.05, className = '', style, children, ...rest }) {
  useInjectedStyle('popcorn-mesh', CSS);
  ...
  return (
    <div
      className={`popcorn-mesh ${className}`.trim()}
      style={{ ...style, background: meshGradient(colors, reduced ? 0.15 : t) }}
      {...rest}
    >
      {children != null && <div className="popcorn-mesh-content">{children}</div>}
    </div>
  );
}
