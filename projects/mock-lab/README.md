...
1. **研究室（Gallery）**——把散落的厂商/社区实践沉淀成一份**可检索、可对比、带出处**的中文知识库：
   ...
   - 覆盖 WireMock / Mockoon / Prism / MockServer / json-server / Hoverfly / Microcks / 云端 mock，
     多协议（REST / gRPC / GraphQL / 异步消息），以及 Python 的
...
2. **配置工坊（Workshop）**——把「mock 哪些、返回什么」抽成一份统一配置，**即取即用**地生成多端产物
   （也能**反向导入** OpenAPI / g-mock 配置反填路由，形成闭环）：
...
def test_calls_external_api():
    with g_mock.bind_responses(MOCK_SOURCE):       # requests（responses）
        ...  # 调用被测代码；内部 requests 请求会被拦截、按配置离线返回
    # 也有：g_mock.bind_respx(...)（httpx/async）、g_mock.bind_requests_mock(...)（requests-mock）
```
...
还能把同一份配置当**真正的本地 mock 服务**跑起来（纯标准库、零依赖）：

```bash
python -m g_mock serve mocks/g-mock.json --port 8000        # 本地配置起服务
python -m g_mock serve https://config.example.com/g-mock.json   # 远端配置起服务
python -m g_mock load mocks/g-mock.json                     # 打印规整后的配置（自检）
```
