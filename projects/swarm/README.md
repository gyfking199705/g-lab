...
- **任务自适应拓扑**：按需求类型选不同协作结构（兑现调研「别一套硬套」）——**编码=单线程深链**（深且顺序）、**调研=广度并行**（3 路并行子问题）、**决策=决策框架**（维度/权重/打分）、**写作=精简管线**，工作区头显示当前拓扑名。
- **结论可导出**：跑完一单可**复制结论 / 复制全文 Markdown / 下载 .md**——含元信息（路由·拓扑·预估）、最终结论与逐角色协作过程，做完即可带走。
