...
- **成本可预测**：派单前就预估 **步数 / 波次 / token / 花费**（按所选模型计价），输入框下与工作区头都显示——回应「多智能体 ~15× token、只对高价值任务划算」。
- **路由快路径**：单一清晰意图（如「翻译这句话」）走 **2 步快路径**（执行者→汇总者），跳过多路调研/规划/返工，省 token；复杂需求才走全量编排（Bedrock 式分级）。
...
接真实大模型时各 agent 的产出**流式逐字显示**（SSE），代理不支持流式会自动回退整体返回。
