...
- 应用首次打开会弹一个**登录优先欢迎页**：主按钮「用 Google 登录并同步」+ 次选「先本地试用」。它只在**全新访客**（没设过同步、本地无 `welcome-seen` 标记）出现一次。
- 「用 Google 登录」的行为取决于 `DEFAULT_CLIENT_ID`：
  - **已写死 Client ID** → 直接弹 Google 授权、自动开启同步、拉取你 Drive 的数据（**一键登录**）。
  - **未写死** → 打开上面的设置向导引导你配置（适合自用）。
- **想让「任何人」打开你的站点都能登录存自己的 Drive**，需要两步（都在你的 Google Cloud 项目里，我无法代办）：
  1. 把你的 Client ID 写死进 `DEFAULT_CLIENT_ID`（方案 A）。
  2. 在 **OAuth 同意屏幕**把发布状态从「测试」改为 **「发布到生产」**（Publish app）。`drive.file` 属于非敏感范围，通常**无需 Google 完整审核**即可对外，用户首次可能看到一次「未验证应用」提示，继续即可；要去掉该提示再走品牌验证。
  - 完成后：任何访客打开 → 欢迎页点「登录」→ 用**他们自己的** Google 账号授权 → 数据进**他们自己的** Drive。彼此隔离，你看不到别人的数据。
