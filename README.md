...
- `sync-client-id` / `sync-auto` / `sync-sig` — 云同步的本机配置（Client ID / 自动开关 / 内容签名）；**仅本机，不纳入备份与同步**

侧边栏底部可**一键导出/导入备份**，或接入 **Google Drive 云同步**：浏览器侧 OAuth、最小权限 `drive.appdata`，
数据写进你自己 Drive 的应用隐藏目录、**无后端**。勾选「自动同步」后改动会防抖自动上传、连接时按内容签名三向对账
（详见 [`sync/README.md`](sync/README.md)）。备份与同步内容均**不含 AI Key**。
