...
### 0. 🏠 首页看板
打开即见全局：今日快览(习惯/日程/逾期) + 今日行动(可就地打卡/勾选) + **进展·趋势**(每个模块一张卡，含迷你趋势图)。**点任意卡片进入该模块的「大盘」**——炫酷的分析视图：渐变英雄区 + 多指标 KPI + 趋势图(手写 SVG 线/柱) + **预测**(如理财预计达成年数、减脂预计达成日、本月预计支出、清单预计读完天数) + 洞察。大盘引擎为纯函数 `app/analytics.js`(已单测)，渲染器 `app/BigBoard.jsx`。各模块的「融合层」，只读/写各模块数据、不单独存储。
...
│   ├── PapersReader.jsx     #   React 组件（推荐 feed/每日精选/阅读清单/AI 总结弹窗），复用 core/ui + learning/ai
│   ├── proxy/               #   可选 arXiv 代理 Worker（解决 CORS，无后端/无 key）
│   ├── arxiv.test.js / calc.test.js / bootstrap.jsx / index.html / README.md
├── ledger/                   # 🧾 记账模块（收支流水/月度预算/支出趋势）
│   ├── calc.js              #   纯函数（月度汇总/分类占比/支出趋势/预算/结余），可单测
│   ├── LedgerPlanner.jsx    #   React 组件（快速记账+月汇总+SVG 趋势+分类条），复用 core/ui
│   ├── bootstrap.jsx / index.html / calc.test.js / README.md
├── learning/                 # 📚 AI 学习计划站模块
...
部署到 GitHub Pages 后直接访问站点首页；各模块也有独立演示页：日程 `/schedule/`、目标 `/goals/`、习惯 `/habits/`、减脂 `/cut/`、论文 `/papers/`、记账 `/ledger/`、学习站 `/learning/`、健身 `/fitness/`、项目规划 `/project/`、财富规划器 `/savings/`。
...
node scripts/build.mjs        # 产出 dist/{app,learning,fitness,project,savings,schedule,goals,habits,cut,papers,ledger}.js
```
...
cd papers   && node --test    # arXiv 查询构造/Atom 解析 + 阅读进度/连续/分类/AI prompt/每日精选（19 个用例）
cd ledger   && node --test    # 月度汇总/分类占比/支出趋势/预算/结余（10 个用例）
```
...
- `ledger-planner` — 记账（收支流水 + 月度预算），结构带版本号 `v`
